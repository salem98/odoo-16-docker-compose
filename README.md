...
``` bash
curl -s https://raw.githubusercontent.com/salem98/odoo-16-docker-compose/master/run.sh | sudo bash -s odoo-one 10016 20016
```
and/or run the following to set up another Odoo instance @ `localhost:11016` (default master password: `lemdev.tech`):

``` bash
curl -s https://raw.githubusercontent.com/salem98/odoo-16-docker-compose/master/run.sh | sudo bash -s odoo-two 11016 21016
```
...
* Default database password (**admin_passwd**) is `lemdev.tech`, please change it @ [etc/odoo.conf#L60](/etc/odoo.conf#L60)
